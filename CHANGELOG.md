...
- **`extra_tags` 覆盖模型标签**：挂了三轮（R4 §5.7 / R5 §四c / R6 表格末行），本轮由
  项目所有者确认**是有意为之**，与 `title`「传入即最终标题」同一条口径，代码不动。
  结论写进 `docs/INTERNALS.md` §3.2，后续报告不必再提。
- **valence 扁平化对检索的影响**（R6 §四.3）：暂不动。「事件本身愉快但无情绪词给
  0.6~0.7」是给一个此前没有立场的类别定新语义，且与 prompt 里已有的「不要因为事情
  办成了就给 0.6~0.8」直接冲突，回归风险要等下一轮真实回归才验得了。先保留观察。
